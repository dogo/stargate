...
| `reviewer` | the run's isolated worktree | no; emits JSON findings with an explicit `verdict`, which an opt-in severity policy can override (prose `VERDICT:` still accepted) |
...
Full user documentation: [`README.md`](README.md) (1398 lines — it is the reference, do not
duplicate it here). Verified single-vendor configurations: [`examples/`](examples/).

## Repository state

**Both modes are implemented and covered.** 242 tests, all passing (`make test`, exit 0).
